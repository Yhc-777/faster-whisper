...
# word_timestamps=True：在「分词/分字」的粒度上给起止时间。中文等无语种空格的语言在库内
# 会按「完整 Unicode 码点」切分（见 tokenizer split_tokens_on_unicode），因此每行往往是一个字，
# 不是词典里的整词。识别对错仍由 ASR 决定，不会因为是 word-level 就自动更准或合并成整词。
